...
const computePackageInstances = (packages, name, useMostCommon) => {
    ...
    Object.keys(versions).forEach((version) => {
        ...
        packageInstances.forEach((packageInstance) => {
            ...
            if (semver.validRange(packageInstance.requestedVersion) &&
    ...
    packageInstances.forEach((packageInstance) => {
        ...
        // packageInstance.satisfiedBy = new Set(candidateVersions);
...
const getDuplicatedPackages = (json, { includeScopes, includePackages, excludePackages, useMostCommon, }) => {
    const packages = extractPackages(json, includeScopes, includePackages, excludePackages);
    return Object.keys(packages)
        .reduce((acc, name) => acc.concat(computePackageInstances(packages, name, useMostCommon)), [])
        .filter(({ bestVersion, installedVersion }) => bestVersion !== installedVersion);
};
exports.listDuplicates = (yarnLock, { includeScopes = [], includePackages = [], excludePackages = [], useMostCommon = false, } = {}) => {
...
exports.fixDuplicates = (yarnLock, { includeScopes = [], includePackages = [], excludePackages = [], useMostCommon = false, } = {}) => {
    const json = parseYarnLock(yarnLock);
    getDuplicatedPackages(json, {
        includeScopes,
        includePackages,
